...
This report provides an overview of the Armed Conflict Location and Event Data Project (ACLED) dataset for Myanmar. This dataset contains information on conflict actors, conflict events, event types, event locations and fatalities dating back to 2010; the data is updated on a monthly basis. 
...
The terminology employed with this report follows what has been laid out in the ACLED codebook; the review of the codebook, as well as ACLED's methodology notes on Myanmar, is highly encouraged. The specific data sources used for this report are:
...
This report would like to highlight the excellent scholarship of the Armed Conflict Location and Event Data Project, as well as their dedication in making this wealth of data public. This report also echoes ACLED's calls for increased protections for journalists and independent reporting. 
...
The GIF  below 2021 conflict events by **month**. The colour of each point indicates the interaction type -- the 6 most common interaction types have been included and less frequent types have been recoded as "Other" -- and the number of fatalities associated with each event is indicated by the size of each point. 
...
There is, in general, a dearth of partners in the parts of the country most affected by conflict. Battles and explosions and remote violence were most prevalent in Sagaing, and, to lesser extents Kachin and Shan North. The specific relationships between conflict events and partner presence will be explored in later sections of this document. 
...
The plot below shows a massive spike in conflict events in 2021, bearing in mind that Myanmar has long been embroiled in a 70-year-long civil war. 
...
### 3.3 Breakdown of conflict events and fatalities by state/region
...
The next two sections summarise conflict events and fatalities by event type and actor type. For the plots below, one should bear in mind that most event types (with the exception of peaceful protests) involved more than one actor, so totalling up each facet will result in a number higher than the total number of events. For instance, rebel groups took part in around 1,250 battles, but many of these battles were between them and state forces and political militias; the same battle will also show up in the count of the other involved actor.

<br>

```{r actor-facet-event-count}
...
actors %>%
  ...
  ggplot(aes(x = count, y = inter1, fill = inter1)) +
  geom_col() +
  ...
  theme(legend.position = "none",
        plot.caption = element_text(hjust = 0.5)) +
  labs(x = "Number of conflict events associated with actor type", 
       y = "Type of actor", 
...
acled %>% 
  ...
  mutate_at(vars(Battles, `Explosions/Remote violence`,  `Violence against civilians`, Protests, Riots, `Strategic developments`),
            ~ as.numeric(.x)) %>% 
  relocate(`Violence against civilians`, .after = `Explosions/Remote violence`) %>% 
  kable(caption = "2021 number of conflict events by event type", format.args = list(big.mark = ","), digits = 0) %>% 
  kable_classic_2() %>% 
...
For instance, under fatalities related to protests, `r filter(actors, inter1 == "protesters") %>% {sum(.$fatalities)}` fatalities were associated with protesters, with the other party involved almost entirely being state forces, with very small numbers being associated with political militias and rioters. 

<br>

```{r actor-facet-event-fatalities}
...
acled %>% 
  ...
  transpose_df() %>% 
  row_to_names(row_number = 1) %>% 
  mutate_at(vars(Battles, `Explosions/Remote violence`, Protests, Riots, `Strategic developments`, `Violence against civilians`),
            ~ as.numeric(.x)) %>% 
  kable(caption = "2021 fatalities by conflict event type", format.args = list(big.mark = ","), digits = 0) %>% 
  kable_classic_2() %>% 
  footnote(general = "Data source: Armed Conflict Location & Event Data Project (ACLED); acleddata.com",
           general_title = "")
```
...
As mentioned, it is difficult at this juncture to state how many of the associated fatalities were persons killed by these groups and how many were members of these groups who have been killed. Still, it is worthwhile to plot which actors have been the most "violent", at least in regards to the outcomes of their actions. 
...
```{r fatalities-actors-errorbarh, fig.height=7}
...
actors %>% 
  ...
  kable(caption = "Top actors in terms of fatalities associated with them; actors associated with <100 fatalities have been lumped together as 'Other'",
...
kable(top_actors, caption = "Top 5 actors by actor type", format.args = list(big.mark = ",")) %>% 
...
actors %>% 
  ...
  theme_void() +
  labs(fill = "number of\n conflict actors",
       title = "Distribution of conflict actors at township level",
       caption = "Data source: Armed Conflict Location & Event Data Project (ACLED); acleddata.com")
...
### 5.1 Updating the MIMU-HARP conflict index
...
A conflict score was calculated using ACLED data: its construction was similar to the conflict index in the MIMU-HARP Vulnerability Analysis, with the notable omission of data on displacement -- this has been requested and the score will be updated once that data has been made available. 
...
<br>
As can be seen from the largely flat blue line, which is just a simple linear model (the results of this model are printed below), the number of beneficiaries per township does not predict the incidence of conflict (represented by the conflict score, where higher values indicate higher incidence of conflict). There is no demonstrable relationship -- positive or negative -- between the overall conflict index and the numbers of beneficiaries and partners per township:
...
For instance, violence against civilians is highly correlated with strategic developments (with a coefficient of 0.79) -- this means that in 79% of the townships where strategic developments occurred (the most common strategic developments were arrests and looting/property damage), violence against civilians was also reported. 
...
acled %>%  filter(year == 2021 & event_type == "Strategic developments") %>% 
  ...
  mutate(percentage = round(count / sum(count) * 100, digits = 2)) %>% 
  kable(caption = "Sub-event types under strategic developments, 2021") %>% 
...
A total of `r acled_source %>% distinct(source_long) %>% nrow` news sources were used in the ACLED dataset. About a third of the conflict events in 2021 included in the ACLED dataset were originally reported in the [Democratic Voice of Burma](https://english.dvb.no/). To assess the completeness and reliability of the ACLED dataset, a more thorough analysis of the news sources it makes use of should be conducted. The most common news sources are listed in the plot below. 
...
acled_source %>% 
  ...
  labs(x = "Percentage of conflict events reported by source", 
       y = "", 
       title = "Most common data sources in the ACLED dataset", 
...
* Re-calculate the conflict score once IDP data is obtained and compare these results to previous results recorded by MIMU-HARP in their vulnerability analysis. However displacement is likely to be highly correlated with the other conflict indicators and it is not expected that results will differ substantially. Nevertheless, it was a key component of the MIMU-HARP model and its inclusion will aid in improving comparability. 

* Compare the conflict score and specific conflict indicators to results of the WFP-FAO food insecurity survey. It is important to understand if there are any significant correlations between the two. Filter the conflict data to only include time periods relevant to WFP-FAO's study. Additionally, these conflict score should be cross-referenced with market prices to see if a predictive model can be established. Additionally, begin the comparison of conflict data and data on malnutrition. There will likely be many challenges in obtaining accurate mortality rates of any kind. 
...
### 8.1 Reference table of conflict events aggregated at township level 
...
### 8.2 Reference table by conflict actor
...
```{r fig.height=11} 
conflict_score %>%  
  mutate(score_i = replace(score_i, score_i == 0, NA)) %>% 
  ...
  labs(title = "Conflict score by township 2021",
       ...
       caption = "Calculated from Armed Conflict Location & Event Data Project (ACLED); acleddata.com",
       fill = "conflict score") +
