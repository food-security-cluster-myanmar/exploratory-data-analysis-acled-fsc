...
google_analytics: G-7CZPQJVQVL
---

<!-- Global site tag (gtag.js) - Google Analytics -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-7CZPQJVQVL"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-7CZPQJVQVL');
</script>
...
conflict_score <- read_csv("./data/conflict_score2.csv") %>% 
  ...
  left_join(fsc_2022 %>%  
              ...
              summarise(partners = n_distinct(org_code),
                        beneficiaries = sum(new_beneficiaries)), by = "admin3_pcode") %>% 
  left_join(fsc %>% 
...
acled_new %>% 
  ...
  full_join(fsc_2022 %>%
              group_by(admin3_pcode = admin3_pcode_old) %>% 
              summarise(partners = n_distinct(org_code), 
                        bbeneficiaries = sum(new_beneficiaries)), by = "admin3_pcode")  %>% 
  left_join(townships %>% 
  ...
  geom_point(aes(x = longitude, y = latitude), size = .5, alpha = .25, colour = "red") + 
  geom_sf(data = pcode1_shape, alpha = 0, colour = "black", size = .5) + 
  scale_fill_viridis_c(option = "mako", direction = -1) + 
  theme_void() + 
  labs(title = "Distribution of food security partners and conflict events in 2021-2022",
...
acled %>%  
  ...
  kable(caption = "2021-2022 event types by percentage of total", 
...
### 3.2 Conflict-related fatalities 

Myanmar experienced  more conflict fatalities in 2021 than in the preceding decade. In 2021 there were 11,506 conflict-related deaths. Between 2010 and 2020 the combined number of deaths was 9,404. The violence appears to be intensifying, with 9,383 fatalities as of 31 May 2022. 

Violence against civilians accounted for 1,891 deaths. This was more than the total number of fatalities across all conflict events in 2019. Before 2021, 2019 had been the deadliest conflict year in Myanmar in the past decade.
...
acled_new %>% 
  ...
  labs(x = "", 
       ...
       title = "Fatalities by conflict event type, 2010-2022", 
       caption = "Data source: Armed Conflict Location & Event Data Project (ACLED); acleddata.com")
...
acled_new %>% 
  ...
  facet_wrap(~ event_type, scales = "free_y", ncol = 1) + 
   labs(x = "",
       y = "Number of events", 
       title = "Number of conflict events per month, by type",
...
However, the event type *Violence against civilians* does not encompass all fatal incidents involving non-combatants. If the data is examined through the lens of interaction type, which records the types of actors involved in each conflict event, it shows that `r round((filter(acled_new, year >= 2021 & str_detect(inter_type, "civilian|protester")) %>% {sum(.$fatalities)}) / (filter(acled_new, year >= 2021) %>% {sum(.$fatalities)}) * 100, digits = 2)` percent of fatalities stem from interactions involving non-combatants, that is civilians and protesters. The next chapter deals with a more detailed assessment of the number of fatalities related to non-combatants.
...
acled %>%  
  ...
  arrange(desc(Percentage)) %>% 
  kable(caption = "2021-2022 conflict fatalities by interaction type", format.args = list(big.mark = ",")) %>% 
  kable_classic_2("striped", full_width = FALSE, position = "left") %>%  
  footnote(general = "Data source: Armed Conflict Location & Event Data Project (ACLED); acleddata.com", general_title = "")
...
acled_new %>% 
  ...
  labs(x = "", 
       ...
       title = "2021-2022 conflict events by state/region", 
       caption = "Data source: Armed Conflict Location & Event Data Project (ACLED); acleddata.com")
...
acled_new %>% 
  ...
  geom_col() +
  geom_text(aes(label = comma(stat(y), accuracy = 1), group = admin1), stat = "summary", fun = sum, vjust = -0.5, size = 3) +
  scale_y_continuous(labels = comma, breaks = seq(0, 9000, by = 1000)) +
  theme(axis.text.x = element_text(size = 7, hjust = 1, angle = 60), 
        axis.text.y = element_text(size = 9), 
        legend.text = element_text(size = 5),
        legend.title = element_text(size = 6),
...
Non-combatants were involved in almost one third of conflict-related fatalities in Myanmar in 2021. In total, there were `r filter(acled_new, year >= 2021) %>% {sum(.$fatalities)} %>% format(big.mark = ",")` fatalities, of which `r round(filter(acled_new, year >= 2021 & non_combatant == TRUE) %>% {sum(.$fatalities)}) %>% format(big.mark = ",")` were related to non-combatants. 
...
acled_new %>% 
  ...
  rename(`Sub-event type` = sub_event_type, 
         Events = events, 
         Fatalities = fatalities) %>% 
  kable(caption = "Top 10 sub-event types involving non-combatants in 2021-2022", format.args = list(big.mark = ",")) %>% 
...
Kale Township in Sagaing experienced the highest rate of fatalities involving non-combatants in 2021. In total, 7 per cent of non-combatant-related fatalities occured in Kale, which represents `r filter(acled_new, year >= 2021 & admin3_pcode == "MMR005027" & non_combatant == TRUE) %>% {sum(.$fatalities)}` deaths. Overall, `r filter(acled_new, year >= 2021 & fatalities > 0 & non_combatant == TRUE) %>% distinct(admin3_pcode) %>% nrow()`  townships reported fatalities involving non-combatants in 2021 and 2022. Of the 20 townships with the most non-combatant fatalities, 11 were in the Sagaing Region. 
...
actors_new %>% 
  ...
  mutate_at(vars("Protesters", "Civilians", "State_forces", "Political_militias", "Rebel_groups",
                                        "Rioters", "Identity_militias", "External_other_force"),
            ~ as.numeric(.x)) %>% 
  kable(caption = "2021-2022 number of conflict events by actor type", format.args = list(big.mark = ","), digits = 0) %>% 
  kable_classic_2() %>% 
  footnote(general = "Data source: Armed Conflict Location & Event Data Project (ACLED); acleddata.com",
           general_title = "")
```
...
```{r fatalities-actors-errorbarh, fig.height=7.5}
actors_conf_int <- actors_new %>%  
  filter(year <= 2021) %>% 
...
actors_conf_int %>% 
  left_join(actors_new %>% 
              group_by(actor_simple, inter1) %>% 
              summarise(fatalities = sum(fatalities)), 
            by = "actor_simple") %>% 
  # left_join(actors_new %>% select(actor_simple, inter1) %>% distinct(), by = "actor_simple") %>% 
  ...
  labs(x = "Log of average number of fatalities per conflict event",
      y = "", 
      title = "Which conflict actors are associated with the most fatalities per event?", 
      subtitle = "Only actors in more than 50 conflict events\nSize indicates number of fatalities associated with actor", 
      caption = "Data source: Armed Conflict Location & Event Data Project (ACLED); acleddata.com",
      colour = "", size = "") + 
  guides(size = "none") + 
  theme(plot.caption = element_text(hjust = 0.5), 
...
kable(top_actors, caption = "Top conflict actors in 2021-2022 by actor type", format.args = list(big.mark = ",")) %>% 
...
conflict_score %>% 
  ...
  mutate_at(vars(beneficiaries, battles, fatalities, 
                 violence_against_civilians, protests_and_riots, 
                 strategic_developments, explosions_remote_violence), 
            ~ ifelse(!is.infinite(log(.x)), log(.x), 0)) %>% 
...
conflict_score %>%
  mutate(ben_tot = beneficiaries + beneficiaries_2021) %>%
  mutate(beneficiaries = log(ben_tot)) %>% 
  ggplot(aes(x = beneficiaries, y = conflict_score)) + 
  geom_point() + 
  scale_x_continuous(trans = "log10", labels = comma) +
  geom_smooth(method = "lm") +
  labs(x = "Log of beneficiaries",
       y = "Conflict score",
title = "Relationship between 2021-2022 beneficiaries reached and conflict score at township level",
...
conflict_scatter <- acled_new %>%
  filter(year >= 2021 & sub_event_type != "Peaceful protests") %>% 
  group_by(admin3_pcode) %>% 
  summarise(events = n(),
            fatalities = sum(fatalities)) %>% 
  left_join(conflict_score %>% select(partners, admin3_pcode, admin1 = admin1.x, admin3 = admin3.x), by = "admin3_pcode") %>% 
...
vulmmr %>%  
  ...
  left_join(conflict_score %>% 
            select(conflict_score, admin3_pcode, battles, explosions_remote_violence, 
  ...
  mutate(conflict_score = replace_na(conflict_score, 0)) %>% 
  ggplot(aes(area = area_sown, fill = conflict_score, subgroup = state_name, 
...
conflict_score %>% 
  left_join(townships %>% select(state_name, township_name, admin3_pcode), by = "admin3_pcode") %>% 
  mutate(conflict_score = round(conflict_score, digits = 2)) %>% 
  right_join(pcode3_shape, by = "admin3_pcode") %>%
  st_as_sf() %>%
  ggplot() +
...
[![conflict network graph](./text_mining/conflict_network_graph.png)](https://raw.githubusercontent.com/food-security-cluster-myanmar/exploratory-data-analysis-acled-fsc/master/text_mining/conflict_network_graph.png){target="_blank"}
Click **[here](https://raw.githubusercontent.com/food-security-cluster-myanmar/exploratory-data-analysis-acled-fsc/master/text_mining/conflict_network_graph.png){target="_blank"}** to see the full-sized graph. 
...
acled_new %>%  
  ...
  scale_fill_viridis_c(option = "mako", direction = -1) +
  theme_void() + 
  labs(title = "Conflict events by township 2021-2022",
...
fsc_2022 %>%
  ...
  theme_void() +
  labs(title = "Distribution of food security partners in 2022",
...
acled_new %>%  
  ...
  scale_fill_viridis_c(option = "mako", direction = -1) +
  theme_void() + 
  labs(title = "Conflict fatalities by township 2021-2022",
       caption = "Data source: ACLED; acleddata.com",
       fill = "fatalities") +
  theme(plot.caption=element_text(hjust = 0.2)) +
...
fsc_2022 %>%
  ...
  theme_void() +
  labs(title = "Distribution of food security beneficiaries in 2022",
       caption = "Data source: Food Security Cluster Myanmar",
       fill = "number of\nbeneficiaries") +
  theme(plot.caption=element_text(hjust = 0.2))
